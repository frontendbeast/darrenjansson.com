...
<script>
  ...
  const isLogoInViewport = () => {  
    ...
    const inView = rect.top >= -Math.abs(logo.offsetHeight);
    const className = 'has-logo';

    if(inView != logoInViewPrev) {
      if(inView) {
...
<svelte:window on:scroll|passive={isLogoInViewport} />
